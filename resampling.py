...
def try_classifiers(features, labels, datasetName):
    ...
    plot_accuracy(accuracy_true, datasetName, 'resampling, only text')
    plot_accuracy(accuracy_false, datasetName, 'resampling, all features')
    plot_f1(f1_true, datasetName, 'resampling, only text')
    plot_f1(f1_false, datasetName, 'resampling, all features')
...
def encodeFeatures(X_train, X_test, Y_train, useonlytext):
    ...
    if(useonlytext):
        ...
        return tfidf_train, tfidf_test, Y_train
        ...
        sm = SMOTE(random_state=42, sampling_strategy='not majority')
        final_train, Y_train = sm.fit_resample(final_train, Y_train)

        return final_train, final_test, Y_train
...
def cross_validate(model, modelName, cv, X, Y, useOnlyText):
    ...
    for train_index, test_index in skf.split(X, Y):
        ...
        X_train, X_test, y_train= encodeFeatures(X_train, X_test, y_train, useOnlyText)
...
def plot_accuracy(accuracy_dict, dataset, string):
